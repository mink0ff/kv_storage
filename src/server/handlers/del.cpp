...
namespace myservice::handlers {
    ...
    Del::Del(const userver::components::ComponentConfig& config,
        const userver::components::ComponentContext& context):
        HttpHandlerJsonBase(config, context),
        storage_component_(context.FindComponent<myservice::components::StorageComponent>().GetStorage())
    {}

    userver::formats::json::Value Del::HandleRequestJsonThrow(
        ...
        userver::server::request::RequestContext&) const  
    {
        ...
        auto response_builder = MakeDelKeyResponse(*storage_component_, key);
